 ...
 */
public class FilteredIterator<T, C extends WithGet<T> & WithSize> implements Iterator<T> {
    private final C collection;
    private int cursor = -1;
    private final Predicate<T> filter;

    /**
     * Secondary constructor. Accepts all elements.
     * @param collection which is to be iterated through.
     */
    public FilteredIterator(C collection) {
     ...
     */
    public FilteredIterator(C collection, Predicate<T> filter) {
